...
def initialize_progress(max_iter: int):
    ...
    # testing data
    stats['test_loss_mean'] = np.nan*np.ones(max_iter)
    stats['test_loss_sigma'] = np.nan*np.ones(max_iter)
    stats['test_corr_mean'] = np.nan*np.ones(max_iter)
    stats['test_corr_sigma'] = np.nan*np.ones(max_iter)
...
def update_test_progress(stats: dict, iter: int, test_loss: List[float] = None, test_corr: List[float] = None):

    if test_loss != None:
        stats['test_loss_mean'][iter] = np.mean(np.array(test_loss))
        stats['test_loss_sigma'][iter] = np.std(np.array(test_loss))
    if test_corr != None:
        stats['test_corr_mean'][iter] = np.mean(np.array(test_corr))
        stats['test_corr_sigma'][iter] = np.std(np.array(test_loss))
...
def test_model(epoch, model, test_loader, mean = None, device = 'cuda'):
    ...
    with torch.no_grad():
        ...
        for drug, protein, affinity in test_loader:
            ...
            # model pass
            pred_aff, attn = model(drug, protein)

            # calculate mean loss for first iteration
            if epoch == 0 and mean != None:
                mean_test_loss = loss(torch.tensor(mean).repeat(*affinity.shape).to(device), affinity).item()
                
            # loss + corr
            test_loss.append(loss(pred_aff, affinity).item())
            test_corr.append(torch.corrcoef(torch.stack([affinity.view(-1), pred_aff.view(-1)]))[0, 1].item())
...
def train_model(model, dataset, hyper_params, output_directory, time_limit = 12):
    ...
    test_loader = DataLoader(dataset = test_data, batch_size=hyper_params['batch_size'], shuffle = True, collate_fn = custom_collate_fn)
    ...
    for e in range(hyper_params['max_iter']):
        ...
        for drug, protein, affinity in train_loader:
            ...
            # model pass
            pred_aff, attn = model(drug, protein) # predicting binding affinity

            # calculating mean predictor loss
            if e == 0:
                mean = dataset.mean
                iter_mean_loss = loss(torch.tensor(mean).repeat(*affinity.shape).to(device), affinity)
                mean_loss.append(iter_mean_loss.item())
            
            # getting loss equation
            fit_loss = loss(pred_aff, affinity) # normal loss
        ...
        if e % 50 == 0 or e == hyper_params['max_iter'] - 1:
            if e == 0:
                print('testing')
                test_results = test_model(e, model, test_loader, dataset.mean, device)
                
                stats = update_test_progress(stats, iter = e, test_loss = test_results['test_loss'], 
                                         test_corr = test_results['test_corr'])
                stats = update_mean_predictor(stats, test_results['mean_test_loss'])
            else:
                test_results, mean_loss_test = test_model(e, model, test_loader)
                stats = update_test_progress(stats, iter = e, test_loss = test_results['test_loss'], 
                                             test_corr = test_results['test_corr'])
        ...
        if e % 50 ==0 or time.time() - start_time > 60*60*(time_limit - 0.25):
