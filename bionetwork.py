...
class small_dataset(data.Dataset):
    """ Simple dataset containing only the data for the specific cell line / split. """

    def __init__(self, data_list):
        """ Builds dataset from list containing tuples of (drug, dose, TF_output) """
        self.data = data_list
...
class TF_Data(data.Dataset):
    ...
    """ Builds the dataset for prediction from drug + dosage trial to specific TF activity. """
    ...
    def __init__(self, trials_to_tf, trials_metadata, drug_fingerprint_dict):
        ...
        Requires the following datasets:
            - trials_to_tf: class pd.DataFrame in which the indexes are the trial identifiers and columns are
              the transcription factor activity per trial
            - trials_metadata: class pd.DataFrame in which the indexes are the trial identifiers and columns are
              relevant metadata such as the drug, dosage, time, cell_line, train/test split
            - drug_fingerprint_dict: dictionary-like object such as h5py file that returns the ECFP4 fingerprint
              of drugs as 1d array
    ...
    def __getitem__(self, index):
        """ Returns the dosage and drug from the input and the TF activity as output"""
    ...
    def get_cell_line_indices(self, cell_line):
        """ Returns the index of samples associated with a specific cell line """

        return list(self.cell_line_dictionary[cell_line])

    def get_train_test_indices(self, split):
        """ Returns the index of samples that are for training or testing"""
    ...
    def get_cell_line_w_split_indices(self, cell_line, split):
        """ Returns the index of samples of specific cell line and train or test split """
    ...
    def shuffle(self):
        """ Randomizes dataset in which the y-values are shuffled to create mismatched data between drugs and TF output.
